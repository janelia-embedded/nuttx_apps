...
#include <micro-xrce-dds/client/profile/transport/serial_transport_linux.h>
#include <micro-xrce-dds/client/client.h>
...
#define MAX_BUFFER_SIZE    MR_CONFIG_SERIAL_TRANSPORT_MTU * MAX_HISTORY
...
#endif
{
    ...
    static uint8_t output_best_effort_stream_buffer[MR_CONFIG_SERIAL_TRANSPORT_MTU * MR_CONFIG_MAX_OUTPUT_BEST_EFFORT_STREAMS];
    static uint8_t output_reliable_stream_buffer[MAX_BUFFER_SIZE * MR_CONFIG_MAX_OUTPUT_RELIABLE_STREAMS];
    static uint8_t input_reliable_stream_buffer[MAX_BUFFER_SIZE * MR_CONFIG_MAX_INPUT_RELIABLE_STREAMS];
    int i;
    for( i = 0; i < MR_CONFIG_MAX_OUTPUT_BEST_EFFORT_STREAMS; ++i)
    {
        (void) mr_create_output_best_effort_stream(&session, output_best_effort_stream_buffer + MR_CONFIG_SERIAL_TRANSPORT_MTU * i, comm->mtu);
    }
    for( i = 0; i < MR_CONFIG_MAX_INPUT_BEST_EFFORT_STREAMS; ++i)
    {
        (void) mr_create_input_best_effort_stream(&session);
    }
    for( i = 0; i < MR_CONFIG_MAX_OUTPUT_RELIABLE_STREAMS; ++i)
    {
        (void) mr_create_output_reliable_stream(&session, output_reliable_stream_buffer + MAX_BUFFER_SIZE * i, comm->mtu * history, history);
    }
    for( i = 0; i < MR_CONFIG_MAX_INPUT_RELIABLE_STREAMS; ++i)
    {
